...
Fixed-duration forms (`5'`, `2"`, `5'2"` — minutes/seconds) have been **removed from the grammar entirely** this revision (the `duration`/`fixed`/`minutes`/`seconds`/`fractional`/`tuplet` rules are commented out in `grammar.pest`). This is why `prototype.dsch`, which still uses `5'`-style durations, no longer parses at all — see [Known issues](#known-issues).
...
- **`prototype.dsch`** no longer parses at all, for a more fundamental reason than before: the fixed-duration grammar rules (`duration`, `fixed`, `minutes`, `seconds`, `fractional`, `tuplet`) have been commented out of `grammar.pest` entirely this revision, so `5'`/`2"`-style durations have nowhere to go regardless of rule ordering. It also still uses the pre-`@` `reg` keyword. It documents an older/target DSL shape but is not currently runnable.
...
| Stage | Status |
|-------|--------|
| Parser (grammar + AST) | Complete for the current grammar; `prototype.dsch`'s fixed-duration syntax no longer parses (see Known issues) |
